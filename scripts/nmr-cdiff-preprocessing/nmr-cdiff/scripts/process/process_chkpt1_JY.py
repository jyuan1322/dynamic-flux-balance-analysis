...
def pipe_process(expt, fid):
    """Process Bruker FID using nmrglue."""
    dic, data = ng.bruker.read(fid)
    data = ng.bruker.remove_digital_filter(dic, data)

    # constant amplitude scaling (see proc_13C.com and proc_1H.com)
    amp_scaling = 3.05176e-02 if expt == '13C' else 6.10352e-02
    data = data * amp_scaling

    # Optional line broadening (this doesn't work well w/ nmrglue remove_digital_filter)
    # lb = 1.0 if expt == '13C' else 0.3
    # data = ng.proc_base.em(data, lb=lb)
    lb_hz = 1.0
    sw_hz = dic['acqus']['SW_h']      # Bruker stores this explicitly
    lb_param = lb_hz / sw_hz
    data = ng.proc_base.em(data, lb=lb_param)


    # Optional zero filling
    # zf_factor = 3 if expt == '13C' else 2
    # data = ng.proc_base.zf(data, int(zf_factor * data.shape[-1]))
    zf_exp = 3 if expt == '13C' else 2
    data = ng.proc_base.zf(data, data.shape[-1] * (2 ** zf_exp))

    # FFT - keep as COMPLEX!
    data = ng.proc_base.fft(data)

    # phase (even zeros is explicit)
    ...
    # udic[0]['obs'] = dic['acqus']['BF1']
    # udic[0]['sw']  = dic['acqus']['SW_h']
    # udic[0]['car'] = dic['acqus']['O1'] / dic['acqus']['BF1']
    ...
    return dic_pipe, data  # Complex data!
...
def ng_bl(data, method="median"):
    ...
    if method == "poly":
        ...
        coeffs = np.polyfit(x, spec, 3)
        baseline = np.polyval(coeffs, x)
        spec = spec - baseline

    elif method == "median":
        from scipy.signal import medfilt
        spec = spec - medfilt(spec, kernel_size=501)
