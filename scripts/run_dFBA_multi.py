...
def make_logistic_deriv_fn(df_params: pd.DataFrame, ci: float = 0.95, flip_sign: bool = True,
    ...
    required = ["A", "amp1", "C1", "D1", "amp2", "C2", "D2"]
    for col in required:
        if col not in df.columns:
            raise ValueError(f"Column {col} missing from DataFrame")
    ...
    def evaluate(t: float):
        def row_deriv(row):
            z1 = (t - row["C1"]) / row["D1"]
            e1 = expit(z1)
            comp1_deriv = row["amp1"] / row["D1"] * e1 * (1 - e1)

            z2 = (t - row["C2"]) / row["D2"]
            e2 = expit(z2)
            comp2_deriv = row["amp2"] / row["D2"] * e2 * (1 - e2)

            return comp1_deriv + comp2_deriv

        values = df.apply(row_deriv, axis=1)
...
# This works, but is it appropriate? It seems to be compensating for
# the discrepancy between consumed leucine and produced isovalerate
# and isocaproate.
...
panels = {
    "panel_F1": {
        "rxns": ["FHL2", "ID_90", "ID_36"],
        "labels": ["formate hydrogenase lyase", "formate dehydrogenase", "pyruvate formate lyase"],
        "flip": [],
        "colors": ["#f542ef", "#4ef542", "#cbf542"]
    },
    "panel_F2": {
        "rxns": ["ID_632", "ID_723"],
        "labels": ["formate:tetrahydrofolate ligase", "methylenetetrahydrofolate reductase (ferredoxin)"],
        "flip": [],
        "colors": ["#f542ef", "#4ef542"]
    },
    "panel_F3": {
        "rxns": ["ID_466", "ID_326"],
        "labels": ["carbon monoxide: acceptor oxidoreductase", "CO-methylating acetyl-coA synthase"],
        "flip": [],
        "colors": ["#f542ef", "#4ef542"]
    },
    "panel_F4": {
        "rxns": ["PTA2", "ID_280"],
        "labels": ["phosphate acetyltransferase", "acetate kinase"],
        "flip": [],
        "colors": ["#f542ef", "#4ef542"]
    },
    "panel_a1": {
