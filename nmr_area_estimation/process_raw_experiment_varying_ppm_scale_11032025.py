...
working_dir = "/data/local/jy1008/MA-host-microbiome/UGA_HRMAS"
# log_path = os.path.join(working_dir, "load_spectra_log.txt")
# sys.stdout = open(log_path, "w")  # Redirects all print() output to this file
...
for exp_dir in exp_dirs:
    ...
    print(f"Loaded {exp_name}: {len(ppm)} points ({nucleus})")
...
for nucleus, nuc_dict in spectra.items():
    if len(nuc_dict) == 0:
        continue
    save_path = os.path.join(base_dir, f"spectra_{nucleus}.pkl")
    with open(save_path, "wb") as f:
        pickle.dump(nuc_dict, f)
    print(f"Saved {len(nuc_dict)} {nucleus} spectra to {save_path}")
